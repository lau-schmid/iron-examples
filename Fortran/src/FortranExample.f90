...
INTEGER(CMISSIntg)                :: NumberOfDirichelet,NumberOfTractionNeumann,NumberOfPressureNeumann
  ...
  INTEGER(CMISSIntg)              :: InterpolationType,NumberOfGaussXi,NumberOfComponents                       !! These variables store information to be used in "BASIS BLOCK"  defined by ..
  ...
  DO basisIdx = 1, NumberOfBasis  
    ...
    IF (UsePressureBasis)  then
      j=2        ! j does not have any physical meaning 
    else 
      j=1         
    END IF 

    DO k = 1,j   ! k does not have any physical meaning 
  
      InterpolationType= match_basis(Basis_arguments(4-k+1,basisIdx))
      NumberGlobalXElements=str2int(Mesh_arg4(1,basisIdx))
      NumberGlobalYElements=str2int(Mesh_arg4(2,basisIdx))
      NumberGlobalZElements=str2int(Mesh_arg4(3,basisIdx))
      NumberOfGaussXi = str2int(Basis_arguments(5,basisIdx))
  ...
  CALL cmfe_Field_CreateStart(FieldFibreUserNumber(1),all_Region%Region(1), all_FibreField%FibreField(1),Err)
  CALL cmfe_Field_TypeSet(all_FibreField%FibreField(1),CMFE_FIELD_FIBRE_TYPE,Err)
  CALL cmfe_Field_MeshDecompositionSet(all_FibreField%FibreField(1), all_Decomposition%Decomposition(1),Err)
  CALL cmfe_Field_GeometricFieldSet(all_FibreField%FibreField(1), all_GeometricField%GeometricField(1),Err) 
  CALL cmfe_Field_VariableLabelSet(all_FibreField%FibreField(1),  CMFE_FIELD_U_VARIABLE_TYPE,"Fibre",Err) 
  CALL cmfe_Field_ScalingTypeSet(all_FibreField%FibreField(1),    CMFE_FIELD_ARITHMETIC_MEAN_SCALING,Err)
  ...
  DO DependentFieldIdx = 1, NumberOfDependentField
    ...
    IF  (trim(DependentField_arguments(4,DependentFieldIdx)) == "UNDEFORMED") then
      ...
      CALL cmfe_Field_ParametersToFieldParametersComponentCopy(all_GeometricField%GeometricField(DependentFieldIdx), &
        & match_Dependent_field(DependentField_arguments(2,DependentFieldIdx)),CMFE_FIELD_VALUES_SET_TYPE, &
          & 1,all_DependentField%DependentField(DependentFieldIdx),CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,1,Err)
      IF (NumberOfComponents .ge. 2) then
        CALL cmfe_Field_ParametersToFieldParametersComponentCopy(all_GeometricField%GeometricField(DependentFieldIdx), &
          & match_Dependent_field(DependentField_arguments(2,DependentFieldIdx)),CMFE_FIELD_VALUES_SET_TYPE, &
            & 2,all_DependentField%DependentField(DependentFieldIdx),CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,2,Err)
      END IF 

      IF (NumberGlobalZElements .NE. 0) then
        CALL cmfe_Field_ParametersToFieldParametersComponentCopy(all_GeometricField%GeometricField(DependentFieldIdx), &
          & match_Dependent_field(DependentField_arguments(2,DependentFieldIdx)),CMFE_FIELD_VALUES_SET_TYPE, &
            & 3,all_DependentField%DependentField(DependentFieldIdx),CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,3,Err)
      END IF

    else 

      CALL cmfe_Field_ComponentValuesInitialise(all_DependentField%DependentField(DependentFieldIdx), &
        &  CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,1,str2real(DependentField_arguments(4,DependentFieldIdx)),Err)
          
      IF (NumberOfComponents .ge. 2) then
        CALL cmfe_Field_ComponentValuesInitialise(all_DependentField%DependentField(DependentFieldIdx), &
          & CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,2, str2real(DependentField_arguments(4,DependentFieldIdx)),Err)
            
      END IF 
      IF (NumberGlobalZElements .NE. 0 .AND. NumberOfComponents .ge. 3) then
        CALL cmfe_Field_ComponentValuesInitialise(all_DependentField%DependentField(DependentFieldIdx), &
          &  CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,3, str2real (DependentField_arguments(4,DependentFieldIdx)),Err)
  ...
  DO EquationSetIdx  = 1,NumberOfEquationsSet            !!!! loop for eqaution set
    ...
    CALL cmfe_Equations_SparsityTypeSet(all_Equations%Equations(EquationSetIdx),output_type(Output_arguments(1,1)),Err)
  ...
  DO solverIdx = 1,NumberOfSolver
    ...
    IF (trim(Solvers_arguments(8,solverIdx)) == "ACTIVE") then
      ...
      CALL cmfe_Solver_NewtonJacobianCalculationTypeSet(all_Solver%Solver(solverIdx),CMFE_SOLVER_NEWTON_JACOBIAN_FD_CALCULATED,Err)
    ...
    DO i = 1,NumberOfDirichelet
      ...
      DO nodeIdx=1,SIZE(SurfaceNodes,1)
        ...
        IF(NodeDOmain==ComputationalNodeNumber) THEN
          ...
          ENDDO  ! ComponentIdx=1,3
        END IF ! NodeDOmain==ComputationalNodeNumber
      ENDDO  !! nodeIdx=1,SIZE(SurfaceNodes,1)
