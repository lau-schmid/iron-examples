...
!> Main program
PROGRAM LinearElasticity3DExtensionPlaneStressLagrangeBasis
#ifndef NOMPIMOD
  USE MPI
#endif
  USE OpenCMISS
...
#include "mpif.h"
  ...
  REAL(CMISSRP),        PARAMETER ::    HEIGHT      = 20.0_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    ZERO        = 0.0_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    E1          = 30.0E6_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    E2          = 30.0E6_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    E3          = 30.0E6_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    NU1         = 0.499_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    NU2         = 0.499_CMISSRP
  REAL(CMISSRP),        PARAMETER ::    NU3         = 0.499_CMISSRP
  ...
  CALL cmfe_GeneratedMesh_NumberOfElementsSet(GeneratedMesh,[NumberGlobalXElements,NumberGlobalYElements,NumberGlobalZElements],Err)
  ...
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,1,E1,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,2,E2,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,3,E3,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,4,NU1,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,5,NU2,Err)
  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,6,NU3,Err)
...
!  CALL cmfe_Solver_LibraryTypeSet(SOLVER,CMFE_SOLVER_PETSC_LIBRARY,Err)
  ...
  DO TIMESTEP=1,TIMESTEPS
    ...
    BCDISP = 0.4*WIDTH*(TIMESTEP-1)/TIMESTEPS
    DO node_idx=1,SIZE(RightSurfaceNodes,1)
      NodeNumber=RightSurfaceNodes(node_idx)
      CALL cmfe_Decomposition_NodeDomainGet(Decomposition,NodeNumber,1,NodeDomain,Err)
      IF(NodeDomain==ComputationalNodeNumber) THEN
    ...
    IF (TIMESTEP.LE.9) THEN
      WRITE(filename, "(A30,I1)") "LinearElasticity3DPlaneStress_",TIMESTEP
      filename=trim(filename)
    ELSEIF (TIMESTEP.LE.99) THEN
      WRITE(filename, "(A30,I2)") "LinearElasticity3DPlaneStress_",TIMESTEP
      filename=trim(filename)
    ELSEIF (TIMESTEP.LE.999) THEN
      WRITE(filename, "(A30,I3)") "LinearElasticity3DPlaneStress_",TIMESTEP
      filename=trim(filename)
    ELSE
      WRITE(filename, "(A30,I4)") "LinearElasticity3DPlaneStress_",TIMESTEP
      filename=trim(filename)
    ENDIF
  ...
  CALL cmfe_Fields_ElementsExport(Fields,"LinearElasticity3DPlaneStress","FORTRAN",Err)
...
END PROGRAM LinearElasticity3DExtensionPlaneStressLagrangeBasis
